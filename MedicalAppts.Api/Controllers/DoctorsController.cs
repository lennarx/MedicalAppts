...
namespace MedicalAppts.Api.Controllers
{
    ...
    public class DoctorsController : ControllerBase
    {
        ...
        [HttpGet]
        [Route("{doctorId}/availableTimeFrame")]
        [Produces("application/json")]
        [SwaggerOperation(
        ...
        [ProducesResponseType(typeof(void), StatusCodes.Status400BadRequest)]
        [ProducesResponseType(typeof(void), StatusCodes.Status401Unauthorized)]
        public async Task<IActionResult> GetDoctorsPerSpeciality([FromRoute] MedicalSpecialty specialty)
        {
            var command = new GetDoctorsBySpecialtyQuery(specialty);
            var result = (await _mediator.Send(command))
